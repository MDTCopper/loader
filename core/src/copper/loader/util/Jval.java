...
public class Jval {
    ...
    /** Alias class for storing json maps (objects). Uses {@link LinkedHashMap} to preserve insertion order. */
    public static class JsonMap extends LinkedHashMap<String, Jval> {
        // All methods are inherited; no need for custom putAdd
    }

    /** Alias class for json arrays. Uses {@link ArrayList}. */
    public static class JsonArray extends ArrayList<Jval> {
    ...
    static class Hparser {
        ...
        private Jval readObject(boolean objectWithoutBraces) {
            ...
            while (true) {
                ...
                skipWhiteSpace();
                object.put(name, readValue());   // previously putAdd, now using Map.put
                skipWhiteSpace();
                if (readIf(',')) skipWhiteSpace(); // , is optional
            }
        ...
        public static void save(Jval value, Writer tw, int level, String separator, boolean noIndent) throws IOException {
            ...
            switch (value.getType()) {
                case object:
                    ...
                    for (Map.Entry<String, Jval> pair : obj.entrySet()) {
                        if (!(index++ == 0 && level < 0)) nl(tw, level + 1);
                        tw.write(escapeName(pair.getKey()));
                        tw.write(":");
            ...
            switch (value.getType()) {
                case object:
                    ...
                    for (Map.Entry<String, Jval> pair : obj.entrySet()) {
                        ...
                        if (includeQuotes) {
                            tw.write('\"');
                            tw.write(escapeString(pair.getKey()));
                            tw.write("\":");
                        } else {
                            tw.write(Hwriter.escapeName(pair.getKey()));
                            tw.write(':');
                        }

                        Jval v = pair.getValue();
